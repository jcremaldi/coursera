...
neigh_pred = KNeighborsClassifier(n_neighbors = best_n).fit(X, y)
...
clf_pred = svm.SVC(kernel=str(temp_max_svm)).fit(X, y)
...
loanTree_pred = DecisionTreeClassifier(criterion="entropy", max_depth = best_dep).fit(X, y)
...
LR_pred = LogisticRegression(C=best_c, solver=str(temp_max_lr)).fit(X, y)
